...
public class Matrix implements Cloneable
{
    ...
    Note: I IMPLEMETED REPEAT CONSOLE BECAUSE THE WAY I WROTE IT, IT WAS DIFFICULT TO MAKE IT AESTHETICALLY PLEASING AND PLACE IT WITH DRAWSTRING
	  SO I USED ANOTHER CONSOLE.
    */
    static Console c;
    static Console repeatConsole;
	...
	boolean valid = false;
	String method = "";
	TextInputFile matrixText;
	Matrices userMatrix = new Matrices ();
	Matrices secondMatrix = new Matrices ();
	Matrices copyMatrix;
	while (repeat == 0)
	{
	    c.clear ();
	    c.setTextColor (Color.red);
	    c.print ("Matrix Testing (get vs getFromFile)\n\n");
	    c.setTextColor (Color.blue);
	    c.print ("Please enter a VALID method name (get or getFromFile) to create Matrix: ");
	    method = c.readString ();
	    valid = false;
	    while (valid == false)
	    {
		if (method.equalsIgnoreCase ("get") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("\nUser Matrix\n");
		    c.setTextColor (Color.blue);
		...
		else if (method.equalsIgnoreCase ("getFromFile") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("\nText Matrix\n");
		    c.setTextColor (Color.blue);
		    c.print ("Please enter a VALID Matrix text file name: ");
		    matrixText = new TextInputFile (c.readString ());
		    userMatrix.getFromFile (matrixText);
		    valid = true;
		}
		else
		{
		    c.setTextColor (Color.blue);
		    c.print ("\nYou have entered an INVALID method name format.\n");
		    c.print ("Please re - enter VALID method name from MATRICES to act upon given Matrix: ");
		    method = c.readString ();
		}
	    }
	    c.setTextColor (Color.red);
	    c.print ("\n\nMethod Testing (Omit Constructor, print, get and getFromFile)");
	    c.setTextColor (Color.blue);
	    c.print ("\nPlease enter a VALID method name from MATRICES to act on given Matrix: ");
	    method = c.readString ();
	    c.clear ();
	    valid = false;
	    while (valid == false)
	    {
		if (method.equalsIgnoreCase ("clone") == true)
		{
		    if (userMatrix.row != 0 && userMatrix.column != 0)
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Clone Result\n\n");
			c.setTextColor (Color.blue);
			copyMatrix = (Matrices) userMatrix.clone ();
			c.print ("Please enter a DOUBLE value into position (0,0) of the Copy Matrix: ");
			copyValue = c.readDouble ();
			userMatrix.print (c, 0, 85);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
			copyMatrix.print (c, 40 * (userMatrix.column) + 50, 85);
			c.drawString ("Copy Matrix", 40 * (userMatrix.column) + 50, 85 + 40 * (userMatrix.row));
			copyMatrix.matrix [0] [0] = copyValue;
			userMatrix.print (c, 0, 85 + 40 * (userMatrix.row) + 50);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row) + 40 * (userMatrix.row) + 50);
			copyMatrix.print (c, 40 * (userMatrix.column) + 50, 85 + 40 * (userMatrix.row) + 50);
			c.drawString ("Changed Copy Matrix", 40 * (userMatrix.column) + 50, 85 + 40 * (userMatrix.row) + 40 * (userMatrix.row) + 50);
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			userMatrix.print (c, 0, 85);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
			copyMatrix.print (c, 150, 85);
			c.drawString ("Copy Matrix", 150, 85 + 40 * (userMatrix.row));
		    }
		    valid = true;

		}
		else if (method.equalsIgnoreCase ("columnAverage") == true)
		{
		    if (userMatrix.row != 0 && userMatrix.column != 0)
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Column Average Result\n\n");
			c.setTextColor (Color.blue);
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			userMatrix.columnAverage ().print (c, 40 * (userMatrix.column) + 50, 45);
			c.drawString ("Column Average Matrix", 40 * (userMatrix.column) + 50, 45 + 40 * (userMatrix.row));
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			userMatrix.columnAverage ().print (c, 150, 45);
			c.drawString ("Column Average Matrix", 150, 45 + 40 * (userMatrix.row));
		    }
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("sameSize") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("Second Matrix\n\n");
		    c.setTextColor (Color.blue);
		    secondMatrix.get (c);
		    c.clear ();
		    if (userMatrix.row != 0 && userMatrix.column != 0)
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Same Size Result\n\n");
			c.setTextColor (Color.blue);
			c.print ("The Status of Same Size comparing both Matrices: " + userMatrix.sameSize (secondMatrix));
			userMatrix.print (c, 0, 85);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
			secondMatrix.print (c, 40 * (userMatrix.column) + 50, 85);
			c.drawString ("Second Matrix", 40 * (userMatrix.column) + 50, 85 + 40 * (secondMatrix.row));
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			userMatrix.print (c, 0, 85);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
			secondMatrix.print (c, 150, 85);
			c.drawString ("Second Matrix", 150, 85 + 40 * (secondMatrix.row));
		    }
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("square") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("Matrix Square Result\n\n");
		    c.setTextColor (Color.blue);
		    c.print ("The Status of the User Matrix being a square: " + userMatrix.square ());
		    userMatrix.print (c, 0, 85);
		    c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("add") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("Second Matrix\n\n");
		    c.setTextColor (Color.blue);
		    ...
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Addition Result\n\n");
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			secondMatrix.print (c, 40 * (userMatrix.column) + 50, 45);
			c.drawString ("Second Matrix", 40 * (userMatrix.column) + 50, 45 + 40 * (secondMatrix.row));
			userMatrix.add (secondMatrix).print (c, 0, 45 + 40 * (userMatrix.row) + 50);
			c.drawString ("Added Matrix", 0, 45 + 40 * (userMatrix.row) + 40 * (userMatrix.row) + 50);
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			secondMatrix.print (c, 150, 45);
			c.drawString ("Second Matrix", 150, 45 + 40 * (secondMatrix.row));
			userMatrix.add (secondMatrix).print (c, 0,300);
			c.drawString ("Added Matrix", 0,300);
		    }
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("sub") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("Second Matrix\n\n");
		    c.setTextColor (Color.blue);
		    ...
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Subtraction Result\n\n");
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			secondMatrix.print (c, 40 * (userMatrix.column) + 50, 45);
			c.drawString ("Second Matrix", 40 * (userMatrix.column) + 50, 45 + 40 * (secondMatrix.row));
			userMatrix.sub (secondMatrix).print (c, 0, 45 + 40 * (userMatrix.row) + 50);
			c.drawString ("Subtracted Matrix", 0, 45 + 40 * (userMatrix.row) + 40 * (userMatrix.row) + 50);
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			secondMatrix.print (c, 150, 45);
			c.drawString ("Second Matrix", 150, 45 + 40 * (secondMatrix.row));
			userMatrix.sub (secondMatrix).print (c, 0, 300);
			c.drawString ("Subtracted Matrix", 0, 300);
		    }
		    valid = true;
		}

		else if (method.equalsIgnoreCase ("minor") == true)
		{
		    if (userMatrix.row != 0 && userMatrix.column != 0)
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Minor Result\n\n");
			c.setTextColor (Color.blue);
			c.print ("Please enter a VALID POSITIVE INTEGER row value within the matrix to remove: ");
			row = c.readInt ();
			c.print ("Please enter a VALID POSITIVE INTEGER column value within the matrix to remove: ");
			column = c.readInt ();
			userMatrix.print (c, 0, 100);
			c.drawString ("User Matrix", 0, 100 + 40 * (userMatrix.row));
			userMatrix.minor (row, column).print (c, 40 * (userMatrix.column) + 50, 100);
			c.drawString ("Minor of Matrix", 40 * (userMatrix.column) + 50, 100 + 40 * (userMatrix.minor (row, column).row));
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			userMatrix.print (c, 0, 100);
			c.drawString ("User Matrix", 0, 100 + 40 * (userMatrix.row));
			userMatrix.minor (row, column).print (c, 150, 100);
			c.drawString ("Minor of Matrix", 150, 100 + 40 * (userMatrix.minor (row, column).row));
		    }
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("transpose") == true)
		{
		    if (userMatrix.row != 0 && userMatrix.column != 0)
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Transpose Result");
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			userMatrix.transpose ().print (c, 40 * (userMatrix.column) + 50, 45);
			c.drawString ("Transpose Matrix", 40 * (userMatrix.column) + 50, 45 + 40 * (userMatrix.column));
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			userMatrix.transpose ().print (c, 150, 45);
			c.drawString ("Transpose Matrix", 150, 45 + 40 * (userMatrix.column));
		    }
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("determinant") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("Matrix Determinant Result");
		    userMatrix.print (c, 0, 45);
		    c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
		    c.drawString ("Determinant Value: " + userMatrix.determinant (), 0, 45 + 40 * (userMatrix.row) + 25);
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("substitute") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("Second Matrix\n\n");
		    c.setTextColor (Color.blue);
		    ...
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Substitute Result\n\n");
			c.setTextColor (Color.blue);
			c.print ("Please enter a VALID POSITIVE INTEGER column value within the first matrix: ");
			column = c.readInt ();
			userMatrix.print (c, 0, 85);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
			secondMatrix.print (c, 40 * (userMatrix.column) + 50, 85);
			c.drawString ("Second Matrix", 40 * (userMatrix.column) + 50, 85 + 40 * (secondMatrix.row));
			userMatrix.substitute (column, secondMatrix).print (c, 0, 85 + 40 * (userMatrix.row) + 50);
			c.drawString ("Substitute Matrix", 0, 85 + 40 * (userMatrix.row) + 40 * (userMatrix.row) + 50);
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			secondMatrix.print (c, 150, 85);
			c.drawString ("Second Matrix", 150, 85 + 40 * (secondMatrix.row));
			userMatrix.substitute (column, secondMatrix).print (c, 0, 300);
			c.drawString ("Substitute Matrix", 0, 300);
		    }
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("split") == true)
		{
		    if (userMatrix.row != 0 && userMatrix.column != 0)
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix split Result\n\n");
			c.setTextColor (Color.blue);
			c.print ("Please enter a VALID POSITIVE INTEGER column value within the matrix to split from: ");
			column = c.readInt ();
			userMatrix.print (c, 0, 85);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
			userMatrix.split (column).print (c, 40 * (userMatrix.column) + 50, 85);
			c.drawString ("Split Matrix", 40 * (userMatrix.column) + 50, 85 + 40 * (userMatrix.row));
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			userMatrix.print (c, 0, 85);
			c.drawString ("User Matrix", 0, 85 + 40 * (userMatrix.row));
			userMatrix.split (column).print (c, 150, 85);
			c.drawString ("Split Matrix", 150, 85 + 40 * (userMatrix.row));
		    }
		    valid = true;
		}
		else if (method.equalsIgnoreCase ("concatenate") == true)
		{
		    c.setTextColor (Color.red);
		    c.print ("Second Matrix\n\n");
		    c.setTextColor (Color.blue);
		    ...
		    {
			c.setTextColor (Color.red);
			c.print ("Matrix Concatenate Result\n\n");
			userMatrix.print (c, 0, 45);
			c.drawString ("User Matrix", 0, 45 + 40 * (userMatrix.row));
			secondMatrix.print (c, 40 * (userMatrix.column) + 50, 45);
			c.drawString ("Second Matrix", 40 * (userMatrix.column) + 50, 45 + 40 * (secondMatrix.row));
			userMatrix.concatenate (secondMatrix).print (c, 0, 45 + 40 * (userMatrix.row) + 50);
			c.drawString ("Combined Matrix", 0, 45 + 40 * (userMatrix.row) + 40 * (userMatrix.row) + 50);
		    }
		    else
		    {
			c.setTextColor (Color.red);
			...
			secondMatrix.print (c, 150, 45);
			c.drawString ("Second Matrix", 150, 45 + 40 * (secondMatrix.row));
			userMatrix.concatenate (secondMatrix).print (c, 0,300);
			c.drawString ("Combined Matrix", 0, 300);
		    }
		    valid = true;
		}
		else
		{
		    c.setTextColor (Color.red);
		    c.print ("Method Testing (Omit Constructor, print, get and getFromFile)\n\n");
		    c.setTextColor (Color.blue);
		    c.print ("You have entered an INVALID method name format.\n");
		    c.print ("Please re - enter VALID method name from MATRICES to act upon given Matrix: ");
		    method = c.readString ();
		    c.clear ();
		}
	    }
	    repeatConsole = new Console ();
	    repeatConsole.setTextColor (Color.red);
	    repeatConsole.print ("Repeat Option\n\n");
	    repeatConsole.setTextColor (Color.blue);
	    repeatConsole.print ("Please enter a 0 (to restart program) or any other key to end program: ");
	    repeat = repeatConsole.readInt ();
	    repeatConsole.clear ();
	    repeatConsole.close ();
	    //empty array
	}
...
columnAverage - Based on the CURRENT Matrix values, returns a new MATRIX with the same number of rows as the current matrix but with
		...
		Note: EditChecks and rounds floating DOUBLE values accordingly
...
minor - Given an INTEGER type row and column parameter value, returns a NEW MATRICES type object containing a new Matrix that is the exact same
	...
	Note: Starting row and column value is 0 to the end (row - 1 or column - 1)
...
substitute - Given an INTEGER type column value + a MATRICES type object with Matrix (rowx1), returns a new MATRICES type object containing a Matrix that has the
	     ...
	     Note: Starting column value is 0 to the end (column - 1)
...
*/
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==--=-=-=-=-=-=-=-==-=-=-=-=-=--=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-
class Matrices implements Cloneable
{
    //DATA FIELDS
    protected int row = 0;
    ...
    */
	...
	while (this.row < 0)
	{
	    c.print ("Please enter a POSITIVE INTEGER row number: ");
	    this.row = c.readInt ();
	    c.print ("\n");
	}
	while (this.column < 0)
	{
	    c.print ("Please enter a POSITIVE INTEGER column number: ");
	...
	for (int i = 0 ; i < this.row ; i++)
	{
	    ...
	    {
		c.print ("Please enter a DOUBLE value in the Matrix at position (" + i + ", " + a + "): ");
    ...
    */
	...
	for (int i = 0 ; i < this.row ; i++)
	{
	    for (int a = 0 ; a < this.column ; a++)
	    {
		c.drawString (this.matrix [i] [a] + "", xValue + 40 * a, yValue);
	    }
	    yValue = yValue + 40;
	}
    }
    ...
    */
	...
	for (int i = 0 ; i < this.row ; i++)
	{
	    ...
	    {
		firstIndex = (this.matrix [i] [a] + "").indexOf (".") + 1;
		firstValue = (this.matrix [i] [a] + "").substring (firstIndex).length ();
		if (firstValue >= finalValue)
		    finalValue = firstValue;
		average = average + this.matrix [i] [a];
	    }
	    average = (Math.round (average * (Math.pow (10, finalValue))) / (Math.pow (10, finalValue))) / this.column;
    ...
    */
	...
	if (sameSize (secondMatrix) == true)
	{
		...
		{
		    firstIndex = (this.matrix [i] [a] + "").indexOf (".") + 1;
		    secondIndex = (secondMatrix.matrix [i] [a] + "").indexOf (".") + 1;
		    firstValue = Math.pow (10, (this.matrix [i] [a] + "").substring (firstIndex).length ());
		    secondValue = Math.pow (10, (secondMatrix.matrix [i] [a] + "").substring (secondIndex).length ());
		    if (firstValue >= secondValue)
			multiplier = firstValue;
		    else
			multiplier = secondValue;
		    newMatrix.matrix [i] [a] = (Math.round ((this.matrix [i] [a] + secondMatrix.matrix [i] [a]) * multiplier) / multiplier);
		}
	    }
	}
	return newMatrix;
    ...
    */
    ...
    Purpose: Given an INTEGER type row and column parameter value, returns a NEW MATRICES type object containing a new Matrix that is the exact same
	     ...
	     - returns a MATRICES type object containing a data fields and methods exact same as the CURRENT Matrices (see Method clone) if invalid
	       INTEGER (row + column) parameter values are entered.
	     Note: Starting row and column value is 0 to the end (row - 1 or column - 1)
    Input: An INTEGER row value (Range: 0 to row - 1) and an INTEGER column value (Range: 0 to column - 1)
    ...
    */
    ...
    Purpose: Given an INTEGER type column value + a MATRICES type object with Matrix (rowx1), returns a new MATRICES type object containing a Matrix that has the
	     ...
	     - returns a MATRICES type object that has an EXACT copy of the CURRENT MATRIX if the MATRICES type value's column number and the CURRENT Matrix row number are
	       not equal, if the column number is not within range of the CURRENT Matrix, or if the MATRICES type parameter value's Matrix row number is not 1.
	     Note: Starting column value is 0 to the end (column - 1)
    Input: An INTEGER type column value (Range: 0 to column - 1) + MATRICES type value
    ...
    */
    ...
    Purpose: Given an INTEGER type column value, returns a MATRICES type object containing a Matrix that is a section of the CURRENT Matrix
	     ...
	     - returns a MATRICES type object that is an EXACT copy of the CURRENT Matrix ONLY if an INVALID column number is entered
	     Note: Starting column value is 0 to the end (column - 1)
    Input: An INTEGER type column value (Range: 0 to column - 1)
    ...
    */
	...
	if (column >= 0 && column < this.column)
	{
	    ...
	    for (int i = 0 ; i < this.row ; i++)
	    {
		for (int a = column ; a < this.column ; a++)
		{
		    newMatrix.matrix [i] [index] = this.matrix [i] [a];
		    index = index + 1;
		}
		index = 0;
	    }
