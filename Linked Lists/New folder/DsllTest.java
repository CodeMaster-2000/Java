...
public class DsllTest
{
    ...
    */
	...
	while (repeat == 1)
	{
	    ...
	    c.print("Please enter a 1 (to insert an INTEGER + STRING value) or any other INTEGER to use an EMPTY Linked List: ");
	    repeat = c.readInt ();
	    while (repeat == 1)
	    {
		c.clear ();
	    ...
	    c.setTextColor (Color.red);
	    c.println ("\n\nFindBefore STRING Method");
	    c.setTextColor (Color.blue);
	    c.print ("Please enter a STRING value within the Linked List to find: ");
	    data = c.readString ();
	    c.setTextColor (Color.black);
	    outputNode = list.findBefore (data);
	    if (outputNode != null)
		c.println ("The NODE before the NODE containing the STRING: " + outputNode.name);
	    else
	    {
	    ...
	    c.setTextColor (Color.red);
	    c.println ("\n\nFindBefore INTEGER Method");
	    c.setTextColor (Color.blue);
	    c.print ("Please enter an INTEGER value within the Linked List to find: ");
	    value = c.readInt ();
	    c.setTextColor (Color.black);
	    outputNode = list.findBefore (value);
	    if (outputNode != null)
		c.println ("The NODE before the NODE containing the INTEGER: " + outputNode.number);
	    else
	    {
	    ...
	    c.print ("Please enter a STRING value within the Linked List to find: ");
	    data = c.readString ();
	    outputNode = list.find (data);
	    c.setTextColor (Color.black);
	    if (outputNode != null)
		c.println ("The NODE containing the STRING: " + outputNode.name);
	    else
		c.println ("DATA NOT FOUND");
	    c.setTextColor (Color.red);
	    c.println ("\n\nFind INTEGER Method");
	    ...
	    value = c.readInt ();
	    c.setTextColor (Color.black);
	    outputNode = list.find (value);
	    if (outputNode != null)
		c.println ("The NODE containing the INTEGER: " + outputNode.number);
	    else
		c.println ("DATA NOT FOUND");
	    c.setTextColor (Color.blue);
...
class Node
{
    ...
    */
	...
	if (this.nextName != null)
	    result = result + this.nextName.byName ();
	else
	    result = result + "Null";
	return result;
    }
    ...
    */
	...
	if (this.nextNumber != null)
	    result = result + this.nextNumber.byNumber ();
	else
	    result = result + "Null";
	return result;
    }
...
class DoubleSortedLinkedList
{
    ...
    */
	...
	if (this.listName != null)
	    result = "Linked List[Name]: " + this.listName.byName ();
	else
	    result = "Linked List[Name]: Empty";
	return result;
    }
    ...
    */
	...
	if (this.listNumber != null)
	    result = "Linked List[Number]: " + this.listNumber.byNumber ();
	else
	    result = "Linked List[Number]: Empty";
	return result;
    }
    ...
    */
	...
	if (temp != null)
	{
		...
		temp = temp.nextName;
	    if (temp == this.listName && temp.nextName.name.equals(name) == false || temp.nextName == null)
		temp = null;
	}
	return temp;
    ...
    */
	...
	if (temp != null)
	{
	    ...
	    if (temp == this.listNumber &&  temp.nextNumber.number != number || temp.nextNumber == null)
		temp = null;
	}
	return temp;
    }
    ...
    */
	...
	if (this.listName == null || this.listName.name.compareTo (newNode.name) >= 0)
	{
	    newNode.nextName = this.listName;
	    this.listName = newNode;
	}
	else
	{
	    Node nameTemp = this.listName;
	    while (nameTemp.nextName != null && nameTemp.nextName.name.compareTo (newNode.name) <= 0)
		nameTemp = nameTemp.nextName;
	    newNode.nextName = nameTemp.nextName;
	    nameTemp.nextName = newNode;

	}
	if (this.listNumber == null || this.listNumber.number >= newNode.number)
	{
	    newNode.nextNumber = this.listNumber;
	    this.listNumber = newNode;
	}
	else
	{
	    Node numTemp = this.listNumber;
	    while (numTemp.nextNumber != null && numTemp.nextNumber.number <= newNode.number)
		numTemp = numTemp.nextNumber;
	    newNode.nextNumber = numTemp.nextNumber;
	    numTemp.nextNumber = newNode;
	}
    }
